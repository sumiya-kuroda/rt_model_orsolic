...
def main(fname, *, early_licks=False, all_splits=False, control=False, pdf=False):
    ...
    :param bool control: whether to use stim and time only models
    :param bool pdf: whether to plot in pdfs
    """
    ...
    if control == False:
        models = [
            '{}__constant__matern52__proj_wtime__ard'.format(fname)
        ]
    else:
        models = [
            '{}__constant__matern52__proj_wtime__ard'.format(fname),
            '{}__constant__matern52__proj__ard'.format(fname),
            '{}__constant__matern52__wtime'.format(fname)
        ]
